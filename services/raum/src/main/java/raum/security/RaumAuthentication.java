...
public class RaumAuthentication implements Authentication {
    ...
    public RaumAuthentication(UUID orgId, String username, Map<String, List<String>> roles, UUID serviceId) {
        this.orgId     = orgId;
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() {
        return roles.getOrDefault(serviceId.toString(), List.of())
                .stream()
                .flatMap(roleName -> Arrays.stream(RaumRole.values())
                        .filter(r -> r.name().equals(roleName))
                        .findFirst()
                        .map(RaumRole::getPermissions)
                        .orElse(Set.of())
                        .stream()
                )
                .distinct()
