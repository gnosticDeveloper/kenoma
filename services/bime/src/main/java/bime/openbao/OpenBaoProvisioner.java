...
/**
 * Bootstraps Bime's own OpenBao AppRole at startup, mirroring Vassago's provisioner.
 *
 * <p>Bime needs a live vault token so its scheduled jobs (which run with no user JWT
 * and therefore no {@code ReactiveSecurityContextHolder} context) can still request
 * ephemeral per-org database credentials from Raum via the {@code X-Vault-Token} path.
 *
 * <p>Uses a narrow "provisioner" credential (permission only to write bime's own
 * policy/role/secret-id) to create/ensure the real {@code bime} AppRole, mint a
...
public class OpenBaoProvisioner {
    ...
    private static final String POLICY_NAME = "bime-policy";
    private static final String ROLE_NAME = "bime";
    ...
    public OpenBaoProvisioner(
            ...
            @Value("${bime.openbao.provisioner.role-id}") String provisionerRoleId,
            @Value("${bime.openbao.provisioner.secret-id}") String provisionerSecretId,
            @Value("classpath:openbao/bime-policy.hcl") Resource policyResource,
            @Value("${bime.openbao.provisioner.max-attempts:10}") int maxAttempts) {
    ...
    void provision() {
        provisionOnce()
                .retryWhen(Retry.backoff(maxAttempts, Duration.ofSeconds(2))
                        ...
                        .doBeforeRetry(signal -> log.warn("Bime AppRole provisioning attempt {} of {} failed: {}",
                                signal.totalRetries() + 1, maxAttempts, signal.failure().getMessage()))
                        .onRetryExhaustedThrow((spec, signal) -> new IllegalStateException(
                                "Bime AppRole provisioning failed after " + maxAttempts + " attempts", signal.failure())))
                .doOnSuccess(v -> log.info("Bime AppRole provisioned and logged in successfully"))
                .block();
    }
